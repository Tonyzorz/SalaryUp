...
문제의 설명과 같다.
    ...
    public String[] solution(String[] record) {
        ...
        String[][] userAction = new String[record.length][2];
        
        for(int i = 0; i < record.length; i++) {
        	String[] temp = record[i].split(" ");
        	//System.out.println(Arrays.toString(temp));
        	
    		userAction[i][0] = temp[0];
        	userAction[i][1] = temp[1];
        ...
        for(int i = 0; i < userAction.length; i++) {
        	...
        	if(!userAction[i][0].equals("Change")) {
        		String action = userAction[i][0].equals("Enter") ? " 들어왔습니다." : " 나갔습니다.";
        		String name = userId.get(userAction[i][1]) + "님이";
        		
        		answer2.add(name + action);
        	}
        }
        
        String[] answer = new String[answer2.size()];
        answer = answer2.toArray(answer);
